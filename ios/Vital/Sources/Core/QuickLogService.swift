...
/// Undo/"Edit in Vital" act on).
struct QuickLogResult: Equatable {
    let id: String
    let name: String
    let kcal: Int
    let slot: String
...
/// Live `QuickLogServicing` backed by `APIClient`. On a successful log, it
/// also does what every other meal-logging surface does (see
...
struct LiveQuickLogService: QuickLogServicing {
    ...
    func quickLog(text: String) async throws -> QuickLogResult {
        let response = try await api.quickLogMeal(text: text, tz: TimeZone.current.identifier)
        await MainActor.run {
